...
func _discover(path: String = Settings.test_directory()) -> Array:
	...
	while current_name != BLANK:
		...
		if (title.ends_with(".gd") or title.ends_with(".gdc") or title.ends_with(".cs")) and load(title).get("TEST") and title != "res:///addons/WAT/core/test/test.gd" and title != "res://addons/WAT/core/test/Test.cs" and title != "res:///addons/WAT/core/test/test.gdc":
